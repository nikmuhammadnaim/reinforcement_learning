...
class MarkovDecisionProcess_Grid(object):
    """docstring for MarkovDecisionProcess_Grid."""
    ...
    def __init__(self, metadata):
        ...
        def next_state_distribution(self, state, action):
            if action == None:
                ...
                [(self.transition_distribution['forward'], self._move_forward(state, action)),
    ...
    def _expected_value(state, action, values, mdp):
        return sum([prob * values[new_state] for porb, new_state in mdp.next_state_distribution(state, action)])
    ...
    def value_iteration(initial_values, mdp, num_iter=100):
        ...
        for _ in range(num_iter):
            """
            Copy of the updated values to step through the values
    ...
    def policy_extraction(values, mdp):
        policy = {}
        for state in mdp.states:
            # We don't need ot compute the full mdp.R(state) + mdp.discount + ...
            # since mdp.R(state) and mdp.discount are constant given a state
