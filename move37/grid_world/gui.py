...
class GridWorldWindow(object):
    ...
    def __init__(self, metadata):
        ...
        # Create the tkinter IDs for all of the modifiable UI
        ...
        self.ids_row = [[0 for col in range(self.grid_width)] for row in range(self.grid_height)]
        ...
        def _create_buttons_(self):
            ...
            self.btn_value_iteration_1_step = Button(self.frame_value_buttons,
                                                     text='1-Step Value Iteration',
                                                     anchor=W)
            self.btn_value_iteration_1_step.pack(side=LEFT)

            self.btn_value_iteration_100_steps = Button(self.frame_value_buttons,
                                                        text='100-Step Value Iteration',
                                                        anchor=E)
            ...
            self.btn_policy_iteration_1_step = Button(self.frame_policy_buttons,
                                                      text='1-Step Policy Iteration',
                                                      anchor=E)
            self.btn_value_iteration_1_step.pack(side=LEFT)


            stn.btn_policy_iteration_100_steps = Button(self.frame_policy_buttons,
                                                        text='1-Step Policty Iteration',
                                                        anchor=E)
            stn.btn_policy_iteration_100_steps.pack(side=LEFT)

            self.btn_policy_iteration_slow = Button(self.frame_policy_buttons,
                                                    text='Slow Policy Iteration',
                                                    anchor=E)
        ...
        def _create_grid(self):
            for row in range(self.grid_height):
                for col in range(self.grid_width):
                    ...
                    self.ids_rect[row][col] = self.canvas.create_rectangle(col * CELL_SIZE,
                                                                           row * CELL_SIZE,
                                                                           (col+1) * CELL_SIZE,
                                                                           (row+1) * CELL_SIZE,
                                                                           fill=fill,
                    ...
                    if (row,col) in self.terminals:
                        self.canvas.create_rectangle(col * CELL_SIZE + CELL_PADDING,
                                                     row * CELL_SIZE + CELL_PADDING,
                                                     (col+1) * CELL_SIZE - CELL_PADDING,
                                                     (row+1) * CELL_SIZE - CELL_PADDING,
                                                     fill=fill,
                                                     outline='white')
                    self.ids_text[row][col] = self.canvas.create_text(col * CELL_SIZE + CELL_SIZE/2,
                                                                      row * CELL_SIZE + CELL_SIZE/2,
                                                                      text=text,
                                                                      fill='white')
                    self.ids_arrow[row][col] = self.canvas.create_line(0, 0, 0, 0,
                                                                       width=2,
                                                                       arrow=LAST,
                                                                       fill='white')
        ...
        def _compute_color(self, value):
            ...
            if value == 0:
                ...
                g = math.floor(225 if value >= 1.0 else value * 256)
                return '#{:02x}{:02x}{:02x}'.format(0, g, 0)
            elif value < 0:
                r = math.floor(225 if -value >= 1.0 else -value * 256)
